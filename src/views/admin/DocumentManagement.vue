  ...
  <div class="admin-page">
    <el-card>
        ...
        >
          ...
          <template #tip>
            <div class="upload-tip">
              仅支持 Markdown 格式 (.md)，文件大小不超过 50MB
            </div>
          </template>
        </el-upload>
      ...
      <el-table v-loading="loading" :data="documents" stripe style="width: 100%">
        ...
        <el-table-column prop="processingStatus" label="状态" width="120">
          <template #default="{ row }">
            <el-tag :type="getStatusType(row.processingStatus)" size="small">
              {{ getStatusLabel(row.processingStatus) }}
            </el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="200" fixed="right">
          <template #default="{ row }">
            <div class="action-buttons">
              <el-button
                v-if="row.processingStatus === 'FAILED'"
                type="warning"
                size="small"
                text
...
const previewDocumentData = ref<KnowledgeDocumentResponse | null>(null)
const previewUrl = ref<string>('')

const loadDocuments = async () => {
  try {
    loading.value = true
    const res = await adminFetchDocuments()
...
const uploadFile = async (file: File) => {
  ...
  if (fileExtension !== '.md') {
    ElMessage.error('仅支持 Markdown 格式文件')
    return
  }

  try {
    uploading.value = true
    const res = await adminUploadDocument(file)
...
const handleReindex = async (doc: KnowledgeDocumentResponse) => {
  try {
    await ElMessageBox.confirm('确定要重新索引这个文档吗？', '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    })

    await adminReindexDocument(doc.id)
...
const previewDocument = (doc: KnowledgeDocumentResponse) => {

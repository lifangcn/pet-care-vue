      ...
      <template #header>
        <div class="header">
          ...
          <div class="header-actions">
            <el-radio-group v-model="auditStatusFilter" size="default" @change="loadActivities">
    ...
    <el-dialog v-model="detailVisible" title="活动详情" width="700px">
      <div v-if="currentActivity" class="activity-detail">
        ...
        <div class="activity-meta">
          <span>作者：{{ currentActivity.authorName }}</span>
          <span>发布时间：{{ formatTime(currentActivity.createdAt) }}</span>
        </div>
        <div class="activity-description">{{ currentActivity.description }}</div>
      </div>
      <template #footer>
        <div class="dialog-footer">
          <el-button @click="detailVisible = false">关闭</el-button>
          <el-button
            v-if="currentActivity?.auditStatus === 'PENDING'"
            type="success"
            @click="handleAudit(currentActivity, 'APPROVED')"
          >
            通过
          </el-button>
          <el-button
            v-if="currentActivity?.auditStatus === 'PENDING'"
            type="danger"
            @click="handleAudit(currentActivity, 'REJECTED')"
          >
            拒绝
          </el-button>
        </div>
...
import { fetchAdminActivityList, auditActivity } from '@/services/adminAuditService'
import type { AdminActivityItem, AuditStatusOfContent } from '@/services/adminAuditService'
...
const currentActivity = ref<AdminActivityItem | null>(null)
...
const loadActivities = async () => {
  try {
    ...
    const { data } = await fetchAdminActivityList(params)
...
const handleAudit = async (activity: AdminActivityItem, status: AuditStatusOfContent) => {
  try {
    await ElMessageBox.confirm(
      `确定要${status === 'APPROVED' ? '通过' : '拒绝'}活动「${activity.title}」吗？`,
      '提示',
      {
    ...
    await auditActivity(activity.id, status)
...
const viewDetail = (activity: AdminActivityItem) => {
